...
namespace Examination_System.Presentation.AdminForms
{
    public partial class frmShowStudentExamUc : UserControl
    {
        ...
        private void LoadAnswers(int questionId, QuestionType questionType, FlowLayoutPanel panel)
        {
            ...
            // Get the student's answer(s) for the question
            ...
            else if (questionType == QuestionType.MultipleChoice)
            {
                ...
                for (int i = 0; i < dtAnswers.Rows.Count; i++)
                {
                    ...
                    // Check if this answer is one of the student's answers
                    if (studentAnswerRows.Any(row => row["AnswerId"].ToString() == dtAnswers.Rows[i]["Id"].ToString()))
                    {
